...
export default class VirtualClassroom {
  ...
  constructor(public socket: SocketType) {
    ...
    //Listening on new user's connection in current room
    this.socket.on("UserConnected", (userId) => {
      console.log(`${userId} enters current room`);
    });

    //Listening on other user's disconnection and close peer connection
    this.socket.on("UserDisconnectd", (userId) => {});
